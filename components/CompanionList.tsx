          ...
          {companions?.map(({id, name, subject, duration, topic}) => (
            <TableRow key={id}>
              ...
              <TableCell>
                <Link href={`/companion/${id}`}>
                  <div className="flex items-center gap-2">
                    <div className="size-[76px] flex items-center justify-center rounded-lg max-md:hidden" style={{backgroundColor: getSubjectColor(subject)}}>
                      <Image src={`/icons/${subject}.svg`} alt="subject" width={20} height={20} />
                    </div>
                    <div className="subject-badge min-md:hidden">
                      {subject}
                    </div>
                  </div>
                </Link>
